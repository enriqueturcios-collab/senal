...
import { approvePlanRequest, rejectPlanRequest } from '@/actions/subscriptions'

export function AdminPlanActions({ requestId }: { requestId: string }) {
  ...
  async function approve() {
    ...
    const r = await approvePlanRequest(requestId)
  ...
  async function reject() {
    ...
    setLoading(true); setError('')
    const r = await rejectPlanRequest(requestId, note.trim())
    if ('error' in r) { setError(r.error ?? 'Error'); setLoading(false); return }
    setDone(true)
  }
      ...
      {state === 'rejecting' ? (
        <>
          <textarea
            ...
            rows={2}
            className="w-full rounded-xl px-3 py-2 text-[12px] text-signal-text resize-none focus:outline-none"
        ...
        <div className="flex gap-2">
          <button type="button" onClick={() => setState('rejecting')}
          ...
          <button type="button" onClick={approve} disabled={loading}
            ...
            {loading ? 'Activando…' : 'Aprobar y activar ✓'}
          </button>
        </div>
      )}
