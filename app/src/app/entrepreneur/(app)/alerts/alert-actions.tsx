...
export function AlertActions({ ruleId, isActive }: { ruleId: string; isActive: boolean }) {
  ...
  async function toggle() {
    setLoading(true)
    try {
      await fetch(`/api/entrepreneur/alerts/${ruleId}`, {
        method: 'PATCH',
  ...
  async function remove() {
    ...
    setLoading(true)
    try {
      await fetch(`/api/entrepreneur/alerts/${ruleId}`, { method: 'DELETE' })
      router.refresh()
    } finally {
      setLoading(false)
    }
